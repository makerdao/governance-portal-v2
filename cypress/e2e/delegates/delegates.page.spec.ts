...
describe('Delegates Page', () => {
  ...
  it('should find the delegates system info', () => {
    ...
    setAccount(TEST_ACCOUNTS.normal, () => {
      ...
      cy.get('[data-testid="total-aligned-delegates-system-info"]').contains(TESTING_ALIGNED_DELEGATE_COUNT);
  ...
  it('should hide shadow delegates when unchecking the filter', () => {
    ...
    setAccount(TEST_ACCOUNTS.normal, () => {
      ...
      cy.get('[data-testid="delegate-type-filter-show-aligned"]').click();

      // See now 2 delegates
      cy.get('[data-testid="delegate-card"]').should('have.length', TESTING_ALIGNED_DELEGATE_COUNT);
