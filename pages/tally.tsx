...
import { fetchTallyRankedChoice } from 'modules/polling/api/fetchTallyRankedChoice';
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const csvWriter = createCsvWriter({
    path: 'pollz.csv',
    header: [
      { id: 'pollId', title: 'pollId' },
      { id: 'oldWinner', title: 'oldWinner' },
      { id: 'newWinner', title: 'newWinner' },
      { id: 'sameWinner', title: 'sameWinner' },
      { id: 'optionId', title: 'optionId' },
      { id: 'optionValue', title: 'optionValue' },

      { id: 'roundsOld', title: 'roundsOld' },
      { id: 'roundsNew', title: 'roundsNew' },

      { id: 'firstChoiceOld', title: 'firstChoiceOld' },
      { id: 'firstChoiceNew', title: 'firstChoiceNew' },

      { id: 'transferOld', title: 'transferOld' },
      { id: 'transferNew', title: 'transferNew' },

      { id: 'optionIsWinnerOld', title: 'optionIsWinnerOld' },
      { id: 'optionIsWinnerNew', title: 'optionIsWinnerNew' },

      { id: 'optionWasEliminatedOld', title: 'optionWasEliminatedOld' },
      { id: 'optionWasEliminatedNew', title: 'optionWasEliminatedNew' },

      { id: 'numVotersOld', title: 'numVotersOld' },
      { id: 'numVotersNew', title: 'numVotersNew' },

      { id: 'totalMkrParticipationOld', title: 'totalMkrParticipationOld' },
      { id: 'totalMkrParticipationNew', title: 'totalMkrParticipationNew' }
    ]
  });

  const records = await polls.reduce(async (previousPromise, poll) => {
    const acc = await previousPromise;
    if (Object.keys(poll.options).length !== 3) {
      const tallyOld = await fetchTallyRankedChoice(poll.pollId, SupportedNetworks.MAINNET);
      const tallyNew = await fetchTallyRankedChoice(poll.pollId, SupportedNetworks.MAINNET, true);

      Object.keys(poll.options).map(key => {
        const record = {
          pollId: poll.pollId,
          oldWinner: tallyOld?.winner,
          newWinner: tallyNew?.winner,
          sameWinner: tallyOld?.winner === tallyNew?.winner,
          optionId: key,
          optionValue: poll.options[key],

          roundsOld: tallyOld?.rounds,
          roundsNew: tallyNew?.rounds,

          firstChoiceOld: tallyOld?.options[key]?.firstChoice.toString(),
          firstChoiceNew: tallyNew?.options[key]?.firstChoice.toString(),

          transferOld: tallyOld?.options[key]?.transfer.toString(),
          transferNew: tallyNew?.options[key]?.transfer.toString(),

          optionIsWinnerOld: tallyOld?.options[key]?.winner,
          optionIsWinnerNew: tallyNew?.options[key]?.winner,

          optionWasEliminatedOld: tallyOld?.options[key]?.eliminated,
          optionWasEliminatedNew: tallyNew?.options[key]?.eliminated,

          numVotersOld: tallyOld?.numVoters,
          numVotersNew: tallyNew?.numVoters,

          totalMkrParticipationOld: tallyOld?.totalMkrParticipation.toString(),
          totalMkrParticipationNew: tallyNew?.totalMkrParticipation.toString()
        };
        acc.push(record);
      });
    }
