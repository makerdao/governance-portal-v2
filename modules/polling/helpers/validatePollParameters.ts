...
export const ERRORS_VALIDATE_POLL_PARAMETERS = {
    ...
    victoryConditionsNotValid: 'victory_conditions must include a valid condition. Valid conditions are "plurality" or "instant_runoff"',
    victoryConditionsInvalidCombination: 'victory_conditions combination not valid. instant-runoff and plurality can not be combined together.',
...
export function validatePollParameters(params: Record<string, unknown>): [PollParameters|null, string[]] {
  ...
  if (!params.victory_conditions) {
    ...
    const hasVictoryConditionInstantRunOff = params.victory_conditions.find(i => i.type === PollVictoryConditions.instantRunoff)
    const hasVictoryConditionPlurality = params.victory_conditions.find(i => i.type === PollVictoryConditions.plurality)
  ...
  if (!params.result_display) {
    ...
    if(params.input_format === PollInputFormat.singleChoice && params.result_display !== PollResultDisplay.singleVoteBreakdown )  {
     ...
     if(params.input_format === PollInputFormat.rankFree && params.result_display !== PollResultDisplay.instantRunoffBreakdown )  {
