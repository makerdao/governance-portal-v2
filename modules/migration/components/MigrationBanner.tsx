...
export function MigrationBanner(): React.ReactElement | null {
  const link = <Icon name="chevron_right" size={2} sx={{ ml: 2 }} />;

  const { isShadowDelegate, isDelegateV1Contract, isDelegatedToV1Contract } = useMigrationStatus();
  const showDelegationMigrationBanner =
    (isDelegateV1Contract && !isShadowDelegate) || isDelegatedToV1Contract;
