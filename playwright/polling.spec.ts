...
test('Adds polls to review and navigates to review page and votes with the legacy system', async ({
  ...
  // await expect(page.locator('text=/Your Ballot: 1 vote/')).toBeVisible();

  // Click on the navigate
  const reviewButton = page.locator('[data-testid="review-ballot-button"]');
  await reviewButton.waitFor({ state: 'attached' });
  ...
  await expect(page.locator('text=Share your votes')).toBeVisible();
